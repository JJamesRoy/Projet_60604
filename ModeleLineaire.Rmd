...
Dans la figure ci-dessous \@ref(fig:residualsDensityMult) on peut remarquer que la variance entre des valeurs prédites n'est pas du tout constante. Cela étant dit, l'hypothèse d'homoscédasticité est violée.Aussi, la figure \@ref(fig:QQdensiteMultiple) quant a elle mesure la normalité des données. il est possible d'apercevoir dans ce cas que le dernier tiers des données s'éloigne largement de la ligne de normale dans le graphique Q-Q. L'hypothèse de normalité est alors aussi violée. Une solution possible serait de faire une transformation logarithmique des données. Cette transformation permettrait ainsi aux données d'être plus symétrique.
...
```{r QQdensiteMultiple, out.height = "30%", fig.align = 'center', fig.cap = "Graphique QQ - modèle de densité démographique multiple"}
...
*De plus, en supposant que l'indépendance entre le nombre de déplacements `n_tot` et le terme d'erreur, une variation de 1% de la densité de la population serait associée, dans ce modèle, à une variation moyenne de 1.12% du nombre de déplacements total en bixi. Cependant, l'hypothèse d'indépendance n'est pas nécessairement respectée dans ce cas. Pour cette raison, cette interprétation n'est pas totalement véridique et ne sera pas utilisée dans les modèles à venir, puisqu'on sait qu'il y a des variables qui ne sont pas prises en compte, comme le nombre de pistes cyclables, etc. Cela étant dit, on peut tout de même affirmer que l'effet de la densité sur le nombre total de déplacements reste positif.* A VOIR SI J'AJOUTE
...
## Limitation du modèle de densité


# Conclusion

Enfin toutes ces informations sont importantes pour l'OBNL. Elles expliquent en bref, qu'en moyenne, les arrondissements plus densifiés et l'ajout de certaines infrastructures dans les villes augmentent le nombre d'utilisation des bixi, ce qui est cohérent avec notre hypothèse. À cet effet, il est possible de se demander si ça ne serait pas intéressant pour la compagnie de se coordonner avec les planificateurs urbains des villes pour arrimer l'ajout de station de bixi aux futurs projets de développement urbain des arrondissements. Cela étant dit, ça permettrait d'augmenter le nombre de déplacements en bixi et de diminuer le nombre de véhicules non écologiques sur les routes.
