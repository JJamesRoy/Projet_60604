...
Dans cette section, nous allons analyser si des attraits comme des parcs et/ou des espaces sportives ont un impact sur l'utilisation des Bixis.

Pour tester cette question, nous allons produire plusieurs modèles de régression linéaire et faire de l'inférence sur les résultats obtenus.Les données ont été recueillis sur le site de donnes.montreal.ca.
...
#Parcs
#Westmount = 35 parcs
new_data_parc <- data.frame(arrond = c("Longueuil", "Laval"), nb_parc = c(200, 200))
...
dat_nb_parc <- dat_nb_parc %>% filter(arrond != "") %>%
  filter(arrond != "" & arrond != "L'Île-Bizard-Sainte-Geneviève" & arrond != "Pierrefonds-Roxboro")%>%
  mutate(arrond = sub("Service des grands parcs, du Mont-Royal et des sports", "Mont-Royal", arrond)) %>%
  mutate(arrond = ifelse(arrond %in% c("Commission scolaire", "Autre", "Montréal-Ouest"), "autres", arrond)) %>%
  mutate(nb_parc = ifelse(arrond == "Westmount", 35, nb_parc)) %>%
  group_by(arrond) %>%
  summarise(nb_parc = sum(nb_parc))%>%
  bind_rows(new_data_parc)%>%
  mutate(arrond = gsub(" ", "", arrond))%>%
  mutate(arrond = gsub("-", "", arrond))%>%
  mutate(arrond = gsub("[-–]", "", arrond))%>%
  mutate(arrond = gsub("\\s+", "", arrond))%>%
  mutate(arrond = tolower(arrond))
...
data <- left_join(data,dat_nb_parc, by = "arrond")
#combinaisons_uniques <- unique(data[, c("arrond", "nb_parc")])
...
#Longueuil = 100
#Mont-Royal =
new_data_es <- data.frame(arrond = c("Longueuil", "Laval", "Mont-Royal","Westmount","autres"), nb_es = c(100, 100, 100, 100, 100))

nb_arrond_es <- table(data_es$ARROND)
dat_nb_es <- data.frame(arrond = names(nb_arrond_es), nb_es = as.numeric(nb_arrond_es))%>%
  mutate(arrond = ifelse(arrond == "Verdum", "Verdun", arrond)) %>%
  group_by(arrond) %>%
  summarise(nb_es = sum(nb_es))%>%
  filter(arrond != "" & arrond != "L'Île-Bizard-Sainte-Geneviève" & arrond != "Pierrefonds-Roxboro")%>%
  bind_rows(new_data_es)%>%
  mutate(arrond = gsub(" ", "", arrond))%>%
  mutate(arrond = gsub("-", "", arrond))%>%
  mutate(arrond = gsub("[-–]", "", arrond))%>%
  mutate(arrond = gsub("\\s+", "", arrond))%>%
...
data <- left_join(data,dat_nb_es, by = "arrond")
#combinaisons_uniques <- unique(data[, c("arrond", "nb_es")])

#Création du modèle
modele4 = lm(n_tot~nb_parc + nb_es , data = data)
...
Compte-tenu que le nombre de parc, de bibliothèques, de centres communautaires et de centres sportifs peuvent être influencés par la densité de population, il peut être interessant de générer un modèle qui inclut ces interactions.

```{r}

modele2_interactions <- lm(logN_tot ~ logDensity + ParcKm2 + BibliothequeKm2 + Centre_communautaireKm2 + Centre_sportifKm2 + density*ParcKm2 + density*BibliothequeKm2 + density*Centre_communautaireKm2 + density*Centre_sportifKm2, data = dat_mod)

summary(modele2_interactions)
```

En examinant ce modèle, on constate que tous les paramètres sont significatifs sauf pour le paramètre densité. Ce qui est logique, car celui-ci interragi maintenant avec les autres paramètres. D'après ces informations, nous pouvons interpréter qu'un augmentation de un de BibliothqueKm2 augmenterais le nombre total de déplacement d'environ 8.3. On remarque aussi que le R2 augmente en comparaison aux autres modèles incluant la densité pour une valeur de 39.02%.
