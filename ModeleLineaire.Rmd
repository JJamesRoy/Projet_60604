...
Dans cette deuxième partie du projet, nous devons utiliser des modèles de régression linéaire pour répondre à des questions d'un point de vue d'affaires qui permettent d'expliquer la durée des déplacements et les revenus. La compagnie BIXI étant une organisation à but non lucratif, son but n'est pas nécessairement de maximiser ses profits. De plus, dans la partie 1, nous avons remarqué qu'il y a une corrélation presque parfaite entre le nombre total de déplacement, la durée des déplacements et les revenus. Nous avons donc décider de prendre pour cible le nombre total de déplacement. 

Les enjeux environnementaux sont l'un des enjeux les plus importants de notre époque. Chaque compagnie doit contribuer à réduire l'impact des activités humaines sur l'environnement. Bixi peut y contribuer en mettant en place des conditions qui démocratisent d'avantage l'utilisation de leur vélo. Ainsi elle apporterait son grain de sable dans la transition vers une économie verte dans le secteur des moyens de transport. En plus de contributer à la santé et au bien être de ses usagers. Notre réflexion repose sur l'hypothèse qu'un déplacement Bixi de plus, c'est une personne qui à décider de ne pas prendre sa voiture ou sa moto. Ainsi notre objectif est de formuler des questions de recherche en lien avec l'augmentation du nombre total de déplacement. 

À cet égard, on s'intéresse d'une part à savoir comment les évènements affectent le nombre de déplacements. Autrement dit, est-ce que les festivals à Montréal affectent le nombre d'utilisations des Bixi? D'autre part, on s'intéresse aux portraits démographiques de la ville. À savoir, est-ce que la densité de population, les parcs, les lieux culturels et les autres formes d'infrastructures mises en place dans les villes affectent le nombre d'utilisations des Bixi? Bien sûr, chaque modèle présenté aura ses limites et elles seront toutes abordées en temps et lieu. Toutefois, ces questions peuvent revêtir un intérêt significatif pour la compagnie, car elles offrent la possibilité de cibler les zones où l'utilisation du vélo est plus répandue dans la région métropolitaine de Montréal, tout en explorant les opportunités pour étendre ses services en contribuant à la réduction de l'usage des modes de transport non-écologiques.
...
dat_mod = dat %>% select(-name, -wday, -Date) %>% 
...
```
```{r, results='hide'}
#write.csv(dat_mod, file = "bixi_modif.csv")
```
...
dat_mod2 = bixiWithArron_clean_df
```
```{r modifpart3}
dat_mod2$arrond = dat_mod2$arrondissement
dat_mod2 = dat_mod2 %>% mutate(rain_bin = ifelse(rain == 0, 0, 1)) %>% select(-arrondissement)
dat_mod2 = dat_mod2 %>% mutate(wkend = ifelse(wday == "Saturday" | wday == "Sunday",1,0))
col = c("mm", "dd", "mem", "arrond")
dat_mod2[col] = lapply(dat_mod2[col], factor)
library(stringr)
...
dat_mod2$arrond <- str_replace_all(dat_mod2$arrond, " - ", "-")
dat_pop$arrond <- str_replace_all(dat_pop$arrond, "–", "-")

dat_mod2<- dat_mod2 %>% left_join(dat_pop, by="arrond")

write.csv(dat_mod2, file = "bixi_arrond_dens_full.csv")
```
...
La variable à expliquer dans notre analyse est le nombre total de déplacement. Les modèles de régression linéaire reposent entre autre sur l'hypothèse de normalité de la variable cible.
...
hist(dat_mod$n_tot_mo, 
     ...
     main = "")
```
Sur la figure de gauche \@ref(fig:histn), on voit que les données de la variable `n_tot` ne sont pas normales. En lui appliquant une transformation logarithmique, nous obtenons la distribution illustrée sur la figure de droite \@ref(fig:histn). Celle ci devient approximativement normale. Par contre, le fait que le côté gauche de la courbe soit encore un peu relevé, peut impacter l'inférence statistique que nous ferons.
...
En observant le figure \@ref(fig:festRes1) à gauche et celle à droite sur la figure \@ref(fig:festRes1), on voit que les résidus semblent ne pas être normales dans le premier tiers des données. Par la suite, les résidus semblent se rapprocher d'un distribution normale. Or, étant donné que les résidus ne sont pas parfaitement normales, cela peut poser des limitations dans l'analyse et l'inférence des résultats. Il ne semble pas avoir d'hétéroscédasticité dans les résidus.

## Modèle de régression linéaire multiple

Comme expliqué précédemment, nous devons ajouter des variables dans le modèle pour avoir une estimation plus précise. Il serait utile d'ajouter la variable `holiday`, `mm` et `wkend`. En effet, ces variables agissent à la fois sur `festival` et `n_tot`.
...
En contrôlant pour les variables omises, nous observons maintenant un changement dans l'interprétation des résultats. L'effet des festivals semble être négatif et non-significatif. Dans ce modèle, chaque festival diminue le nombre total de déplacement de 2,4% par jour par quartier de façon non-significative. Il semblerait que l'effet passe maintenant par les mois et non par les festivals.
...
```{r QQdensite, out.height = "20%", fig.align = 'center', fig.cap = "Graphique QQ - modèle de densité démographique simple"}
