...
En examinant ce modèle, on constate que tous les paramètres sont significatifs à au moins 95% de confiance sauf pour le paramètre densité et l'interaction entre Density et BibliothequeKm2. Nous pouvons donc supposer que le paramètre densité a perdu une partie de sa signification et son effet a été capturé par les interactions. Puis, son interaction avec Bibliothèque n'est pas assez significative. On peut aussi constater que si logBibliothequeKm2 augmente de 1, alors le log du nombre de déplacement total augmentera de 13.44, puisque que ces variables sont sur l'échelle logarithmique, les valeurs numériques sont démesurées, cependant on peut comprendre qu'il y a un effet positif à l'augmentation du nombre de bibliothèque. Puisque les variables sont sur une échelle logarithmique, il faut aussi comprendre que les variables avec des coefficient négatifs ont ainsi très peu d'impact sur le nombre de déplacement total si elles augmentent de un. On remarque aussi que le R2 augmente en comparaison aux autres modèles incluant la densité pour une valeur de 37.34% ce qui veut dire que ce modèle explique un peu plus la variabilité du nombre total de déplacements que les autres ci-dessus avec la densité.

Suite aux observations de ce modèle, on peut dire que le nombre de parcs, de bibliothèques et de centre commerciaux influencent positivement le nombre de déplacement en Bixi. Il serait donc judicieux pour la compagnie Bixi de pousser la construction de ces infracstructures.

On peut ensuite examiner les résidus avec les figure \@ref(fig:distResDensiteMultipleInt) et \@ref(fig:qqDensiteMultipleInt)
...
D'après l'analyse des résidus, on constate une allure normale des résidus sur le graphique de la densité et les quantiles semblent suivre raisonnablement la droite sur le graphique QQ. Ainsi, la condition de normalité est sensiblement respectée.

Ensuite, il peut être intéressant de construire un modèle avec les données des non-membres seulement pour voir si ceux-ci ont l'habitude ou non de prendre un Bixi pour.
