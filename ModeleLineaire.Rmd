...
On voit que les données de la variable `n_tot` ne sont pas normales, il faudrait les normaliser pour être capable de faire des analyses statistiques à l'aide de régression linéaire.

```{r}
dat_mod = dat_mod %>% mutate(n_tot_mod = log(n_tot))
hist(dat_mod$n_tot_mod)
```
...
y = "Quantiles empiriques") + ggtitle("QQ-Plot Résidus Student")
```
