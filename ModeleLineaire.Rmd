...
```{r fest-res1, out.height = "30%", fig.align='center', fig.cap="Distribution des résidus et QQ-Plot Résidus Student"}
...
modele_densite <- lm(n_tot_mod ~ logDensity + logParcKm2 + logBibliothequeKm2 + logCentre_communautaireKm2 + logCentre_sportifKm2 + n_AM +wkend + mem, data = dat_mod)
...
plot1 <- ggplot(residuals_data, aes(x = Fitted_Values, y = Residuals)) +
  ...
  ylab("Résidus") +
  ggtitle("Graphique des résidus par rapport aux valeurs prédites") +
...
Pour s'assurer de l'homoscédasticité et linéarité du modèle, on peut ensuite examiner les résidus avec les figure a droite \@ref(fig:distResDensiteMultipleInt) et celle a gauche \@ref(fig:distResDensiteMultipleInt).
...
plot1 <- ggplot(data.frame(Résidus = residuals), aes(x = rstudent(modele2_interactions))) + geom_density() + geom_histogram(aes(y = ..density..),
bins = 20, alpha = 0.5)+
xlab("Résidus") +
ylab("Densité")+
