...
Dans un second temps, chaque arrondissement contient plusieurs stations. En d’autres termes, on peut assumer qu’il risque d’y avoir une corrélation intra-arondissement, soit inter-station dans le même arondissement. De plus, il risque d'y avoir des variation intra-station, ou certaine variables comme l'effet d'être membre ou la fin de semaine varie pour une même station. Autrement dit, en plus d’avoir des effets de groupe inter arrondissement nous nous retrouvons avec des effets de station intra arrondissement et intra station, ce qui est problématique dans un cas d’inférence. Pour aborder cette problématique, nous allons utiliser une approche à effets aléatoire, nous permettant ainsi de ne pas utiliser une pléthore de variable explicative pour contrôler les effets fixes de chaque arrondissement tout en considérant la corrélation intra station. 
...
Il est à noter que dans ce cas, ça ne fait pas vraiment de sens d’utiliser une structure de covariance AR1 et ARH1, puisque les données ne sont pas vraiment de sorte longitudinale. Le fait que les stations reviennent plusieurs fois n’est pas lié à une fonction temporelle. Parallelement, comme mentionnée précédmment, il faut bien comprendre que nos données sont formé de sorte a avoir plusieurs stations dans les meme arondissements.Cela étant dit, on considère une structure d'équicorrélation où la structure de corrélation dans chaque stations est la même pour chaque arrondissement. 

Dans la table anova ci-dessous, il est possible de voir qu'on a crée deux modèle, soit le modele 0 qui ne contient pas de structure de corrélation et le modèle 1 qui contient une structure d'équicorrélation pour les stations. Les deux modèles contiennes les même covariable, membre `mem` la fin de semaine `wkend` et les arrondissements `arrond`. En plus, chaque covariable a une intéraction avec les membres, comme on l'a mentionnée dans les anciennes parties, le fait d'être memebre affecte la durée d'utilisation différenement, selon si c'est la fin de semaine ou la semaine. Aussi, le fait d'être membre affecte chaque arrondissement de façon différente. Dans ce cas spécifique, on peut dire que l'interaction entre les membres et les arrondissements représente l'effet d'être membre sur la durée de déplacement sachant que les effets fixes sont différents entre les arrondissements. On peut aussi voir que la valeur p est raisonnable pour toute valeur de significativité et on peut conclure que la corrélation intra-station est significative.
