...
# Modèle linéaire incluant la structure de corrélation sans effets aléatoire

```{r}
dat$arrond <- as.factor(dat$arrond)

cor(dat[dat$arrond == 'Rosemont-La Petite-Patrie', c("mm", "dd", "n_tot")])

mod1 <- gls(dur ~ mem + arrond, correlation = corCompSymm(form = ~ 1|station), data = dat)
summary(mod1)

cor(dat[c("density", "mem", "arrond")])

gls(revenge ~ sex + age + vc + wom + t, correlation = corCompSymm(form = ~1 |
id), data = longitu1)

table(dat$arrond, dat$density, dat$mem)
print(table_result)
```
