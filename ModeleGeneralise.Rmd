...
Tout d'abord, nous voulons déterminer le meilleur modèle pour faire nos interprétations. Nous allons donc comparer les modèles \@ref(eq:mod1) et \@ref(eq:mod2). Le premier modèle peut s'avérer être une simplification correcte du deuxième modèle. C'est-à-dire que les deux modèles ne sont pas significativement différent en terme de qualité d'ajustement. Le premier est un modèle simple qui tient compte uniquement des arrondissements. Le second est un modèle plus complet qui tient aussi compte des membres, du mois, de la catégorie semaine par rapport à la fin de semaine et des précipitations.
...
<!-- Il est possible d'ajuster un modèle simple pour voir les effets des différents arrondissements sur la probabilité d'avoir des stations où le déplacement moyen est supérieur à 15 minutes afin de trouver les arrondissements plus problématiques. Nous allons ajuster ce modèle : -->

\begin{equation}
u15_i = B_0 + \sum_{i \epsilon n}arrond_{i} + e_i (\#eq:mod1)
\end{equation}
Où $j$ est un arrondissement dans la liste des arrondissements $N_s$.


```{r, out.height = "10%", results='hide'}
...
\begin{equation}
u15_i = B_0 + \sum_{j \epsilon N_s}B_jarrond_{i} + B_2mem_i + \sum_{m \epsilon M}B_mmm_{i} + B_3wkend_i + B_4rain\_bin_i + e_i (\#eq:mod2)
\end{equation}
...
\
Nous allons interpréter le top 5 des arrondissements avec le plus de stations uniques (dans notre échantillon de données) (interpréter l'ensemble des coefficients serait redondant et nous manquerons d'espace). Nous allons calculer la probabilité prédite que u15 = 1, donc cela sert à cibler les arrondissements problématiques.
...
Pour conclure, d’une part, nous utilisons la probabilité prédite des déplacements de plus ou moins de 15 minutes comme proxy pour la disponibilité des Bixi dans certains arrondissements. Ce type d’information peut être décisive pour une entreprise comme Bixi. Comme mentionné précédemment, ce proxy est imparfait, il semble néanmoins fournir une idée de quels arrondissements peuvent avoir des problèmes de disponibilité. Toutefois, le fait qu'en général le nombre d'utilisation augmente avec la durée d'utilisation nous laisse croire qu'il y a une bonne gestion de la flotte.

Nous avons également examiné les facteurs influençant l'affluence à différentes périodes de la journée pour ensuite aborder le nombre total d'utilisations des Bixis. Nous constatons que les membres sollicitent davantage les vélos de l'entreprise le matin par rapport à l'après-midi, contrairement aux non-membres. Cette disparité est probablement liée à l'objectif d'utilisation : les non-membres utilisent davantage les Bixis de manière récréative, tandis que les membres les utilisent probablement comme moyen de transport courant (pour se rendre au boulot peut-être). D'ailleurs l'utilisation des bixi des membres est plus élevé en semaine qu'en fin de semaine. Contrairement au non-membre. Sur la base de ces observations, l'entreprise peut orienter ses stratégies de fidélisation. Ces informations peuvent également guider l'entreprise quant aux moments critiques pour la maintenance de leurs stations. L'utilisation d'un modèle prédictif de la météo peut être utile étant que plus il y a de précipitations moins les bixi sont utilisés. Par ailleurs, chacun de ces modèles présente des limitations significatives que nous avons mis en évidence, soulignant l'importance d'améliorer leur précision en considérant simultanément les effets fixes et les interactions.
