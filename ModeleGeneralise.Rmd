...
write.csv(dat_full, "dat_full_m.csv")
```
...
\begin{equation}
u15_i = B_0 + \sum_{j \epsilon Ns}B_jarrond_{i} + e_i (\#eq:mod1)
\end{equation} Où $j$ est un arrondissement dans la liste des arrondissements $N_s$.

Voici ci-bas les statistiques sur l'ajustement du modèle. Nous n'affichons pas les coefficients, car nous attendons de voir si nous devrions les interpréter et cela dépend de la comparaison avec les deux modèles.
...
#summary(mod_u15_1)
summ(mod_u15_1, model.info = FALSE, model.coefs = FALSE)
```
...
\end{equation}
```
Voici ci-dessous les statistiques et les coefficients liés au deuxième modèle.
