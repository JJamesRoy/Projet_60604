...
$$u15_i = B_0 + \sum_{i \epsilon n}arrond_{i} + e_i$$ Où $j$ est un arrondissement dans la liste des arrondissements $N_s$.
...
\
Nous allons donc interpréter le top 5 des arrondissements avec le plus de stations uniques (dans notre échantillon de données). (interpréter l'ensemble des coefficients serait redondant et nous manquerons d'espace)
...
Le modèle inteprété possède des limites. En effet, il est impossible d'isoler l'effet des arrondissements selon si l'abonnements. Il faudrait faire un modèle avec interactions entre `mem` et `arrond`. Par la suite, il serait nécessaire de tester si ce modèle est plus juste que le précédent. Or, par manque d'espace dans ce travail, nous n'allons pas réaliser cette analyse.
...
Pour commencer, voici la distribution des déplacements sur les figures suivantes:\@ref(fig:three-fig) en PM (à gauche) en AM (au centre) et total (à droite):

```{r three-fig, out.height = "90%",fig.align='center', fig.cap= "Histogramme des déplacements en PM et AM"}
...
plot(mean_values_quasi, variance_values_quasi, xlab = "Moyenne", ylab = "Variance",
     main = "",
     xlim = c(0, 10 ), ylim = c(0, max(150)))
...
En analysant le tableau Anova pour le modèle binomial négatif, on observe que, lors de la comparaison des modèles avec ou sans chaque variable, toutes les covariables présentent une significativité statistique lorsqu'on les ajoute 1 a 1. Par conséquent, il est justifiable de rejeter l'hypothèse nulle (H0) et d'affirmer que, dans ce contexte, au moins un $\beta_i$ est différent de 0.
...
Cependant, la justification de ne pas inclure d'interactions dans le modèle repose principalement sur le fait que la plupart des interactions sont associées à la variable d'arrondissement `arrond`. Étant donné que arrond est une variable catégorielle avec près de 20 niveaux, l'introduction d'interactions créerait un grand nombre de coefficients β, ce qui aurait pour effet d'augmenter considérablement la variance du modèle. Une approche alternative serait d'opter pour un modèle avec des effets aléatoires, ce qui nous permettrait de contrôler les effets fixes sans nécessairement introduire une multitude de paramètres additionnels.
...
L'analyse précédente pour le nombre de déplacements en PM peut aussi être effectué pour les déplacements en AM. Pour commencer, examinons la distribution du nombre de déplacements en AM d'après la figure \@ref(fig:three-fig) au centre vue précédemment.

```{r distnAM, out.height = "30%", fig.align = 'center', fig.cap = "Distribution de nAM"}
#frnAM <-(freq(ordered(dat_full$n_AM), plot = TRUE))
```

Comme on peut le constater avec la figure \@ref(fig:three-fig), la distribution n'est pas d'allure normale. Comme n_AM est une variable de dénombrement, la distribution s'approche plus d'une distribution de Poisson, n_AM a une moyenne de 4.3095 et une variance de 31.12.
...
Le modèle ci-dessus comporte quelques limitations. En effet, celui-ci ne comporte pas d'interactions entre les variables. Cependant, nous sommes porter à croire qu'il en existe. En effet, on peut penser qu'il y a un lien entre la durée d'un trajet et le fait d'être membre ou non, car les membres ont accès à des prix avantageux d'après la durée. Puis, entre la durée et l'arrondissement, car la durée d'un trajet peut dépendre de la destination où l'on va. Soit, une interaction entre `mem` et `dur`, puis une avec `dur` et `arrond`.

### Conclusion AM

Pour conclure, les points importants à réaliser pour comprendre comment interragi le nombre de déplacement en AM avec les autres facteurs est que les membres ont tendance à effectuer plus de déplacement en AM que les non-membres et l'arrondissement avec le plus grand achalandage est l'arrondissement du plateau Mont-Royal. Cependant, il serait possible de pousser plus loin l'analyse en ajoutant des interactions et en vérifiant la significativité de celles-ci.
