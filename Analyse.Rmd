...
dat_n_tot_wday <- dat %>%
  ...
  group_by(wday, mem) %>%
  summarise(n_tot = round((mean(n_tot)),digits=2),
            n_AM =  round((mean(n_AM)),digits=2),
            n_PM =  round((mean(n_PM)),digits=2))
...
geom_text(aes(label=n_tot_vs_day), position=position_dodge(width=0.9), vjust=-0.25)+geom_vline(xintercept=7.5, linetype="dotted", color="red", size = 0.8)
